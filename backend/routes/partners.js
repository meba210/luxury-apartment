...
router.get('/sales', requirePartner, async (req, res) => {
  try {
    ...
    if (search) {
      query += ` AND (place LIKE $${paramIndex} OR agent_name LIKE $${paramIndex + 1})`;
      params.push(`%${search}%`, `%${search}%`);
      paramIndex += 2;
    }
    if (status) {
...
router.put('/me', requirePartner, async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE partners
       SET full_name = $1, phone = $2, additional_phone = $3,
           telegram_username = $4, address = $5, company = $6, about = $7
       WHERE id = $8`,
      [
        full_name.trim(),
        phone.trim(),
...
router.put('/me/password', requirePartner, async (req, res) => {
  try {
    ...
    const newHash = await bcrypt.hash(new_password, 10);
    await pool.query('UPDATE partners SET password_hash = $1 WHERE id = $2', [newHash, req.partner.id]);
